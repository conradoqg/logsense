...
func overlay(base, overlay string) string {
    ...
    // Pad to same length
    maxLen := len(bLines)
    if len(oLines) > maxLen { maxLen = len(oLines) }
    for len(bLines) < maxLen { bLines = append(bLines, "") }
    for len(oLines) < maxLen { oLines = append(oLines, "") }
...
func numericStats(field string, vals []float64) string {
    ...
    for _, v := range vals { uniq[v] = struct{}{} }
    uniqCount := len(uniq)
    bins := uniqCount
    if bins < 5 { bins = 5 }
    if bins > 40 { bins = 40 }
