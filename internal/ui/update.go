...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    // Removed StreamTab and FilterTab shortcuts
		...
		case keyMatches(msg, m.keymap.Explain):
			// Trigger OpenAI explanation for the selected log entry
			...
			if idx < 0 || idx >= len(m.filtered) {
				m.lastMsg = "no entry selected to explain"
				return m, nil
			}
			e := m.filtered[idx]
			return m, tea.Batch(
				func() tea.Msg { return explainStartMsg{} },
				func() tea.Msg {
					...
					defer cancel()
					out, err := client.ExplainLog(ctx, e.Raw, e.Fields)
					if err != nil {
						return explainDoneMsg{ok: false, err: err.Error()}
					}
					return explainDoneMsg{ok: true, content: out}
				},
			)
